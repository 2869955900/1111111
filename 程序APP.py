...
def predict_risk(SDMA_ADMA_pos_140, Thymine_pos_150, Phosphocreatine_neg_067, 
    ...
    for i in range(survival_function.shape[0]):
        ax.step(survival_function.columns, survival_function.iloc[i], where="post", label=f"Sample {i+1}")
    ax.set_xlabel('Time')
    ...
    return risk_score, survival_function_path
...
if st.button("Submit"):
    risk_score, survival_function_path = predict_risk(
